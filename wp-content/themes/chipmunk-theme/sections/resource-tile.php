...
<a href="<?php echo esc_url( chipmunk_external_link( $resource_website ) ); ?> "target="_blank" class="<?php echo $tile_classes[chipmunk_theme_option( 'display_resource_as' )]; ?><?php echo $is_column ? ' column column_md-3 column_lg-4' : ''; ?>">
	...
	<div class="tile__content<?php echo ( chipmunk_theme_option( 'display_resource_as' ) == 'tile' ? ( chipmunk_theme_option( 'disable_resource_thumbs' ) ? ' tile__content_primary' : ' tile__content_dimmed' ) : '' ); ?>">
		<div class="tile__info">
			...
			<?php if ( ! chipmunk_theme_option( 'disable_resource_desc' ) and ! empty( $content ) ) : ?>
				<p class="tile__copy"><?php echo esc_html( chipmunk_truncate_string( $content, ( chipmunk_theme_option( 'display_resource_as' ) == 'card_blank' ? 200 : 200 ) ) ); ?><span>&nbsp;<i class="icon icon_arrow-right" aria-hidden="true"></i></span></p>
			<?php endif; ?>
		</div>
